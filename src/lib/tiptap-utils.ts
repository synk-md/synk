...
import { storeImageAsset, type ImageAssetReference } from "@/lib/image-assets"
 ...
 * @returns Promise resolving to the persisted image asset reference
 */
export const handleImageUpload = async (
  ...
  return await new Promise<ImageAssetReference>((resolve, reject) => {
    ...
    const handleLoad = async () => {
      try {
        ...
        const asset = await storeImageAsset(file)
        if (abortSignal?.aborted) {
          handleAbort()
          return
        }
        ...
        cleanup()
        reject(error instanceof Error ? error : new Error("Failed to store image"))
      }
    }
