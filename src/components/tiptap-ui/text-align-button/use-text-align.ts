...
export function canSetTextAlign(
  ...
  // Images don't support justify (there's nothing to "justify" — they're a
  // single block, not flowing text), but left/center/right map directly to
  // the image's own `align` attribute.
  if (isNodeTypeSelected(editor, ["image"])) return align !== "justify"
...
export function isTextAlignActive(
  ...
  if (isNodeTypeSelected(editor, ["image"])) {
    return editor.isActive("image", { align })
  }
...
export function useTextAlign(config: UseTextAlignConfig) {
  ...
  useHotkeys(
    ...
    },
    {
      ...
      // TextAlign already registers this same shortcut as a native
      // ProseMirror keymap; firing it again while the editor is focused
